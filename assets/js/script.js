...
const	lightnessColorsPalette = [
	0, 5.40, 8.10, 10.80, 13.50, 16.20, 18.90, 21.60, 24.30, 27, 29.70, 32.40,
	35.10, 37.80, 40.50, 43.20, 45.90, 48.60, 51.30, 54, 56.70, 59.40, 62.10,
	64.80, 67.50, 70.20, 72.90, 75.60, 78.30, 81, 83.70, 86.40, 89.10, 91.80,
	94.50, 97.20, 100
]
...
function toHslString(fireIntensity) {
	const	hue = Number(document.getElementById('hueInput').value)
	let		lightness = lightnessColorsPalette[fireIntensity]
		...
		for (let column = 0; column < fireWidth; column++) {
			...
			if (debug === true) {
				...
				if (document.getElementById('hueInput').value == 0) {
					...
					html += `<td class="pixel" style="background-color: ${toHslString(fireIntensity)}"`;
					html += `</td>`;
				}
			}
		}
